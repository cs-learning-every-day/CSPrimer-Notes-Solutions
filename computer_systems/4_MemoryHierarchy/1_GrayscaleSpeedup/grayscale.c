...
void grayscale(unsigned char *pixels, int32_t width, int32_t height) {
  ...
  unsigned char lum;
  /*
  * Order of iterations impacts speed
  * By iterating over the row, we access sequentially consecutive
  * areas of memory, which has better cache utilization
  * Originally, this was a column-wise iteration, which is has poor
  * cache utilization
  */
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
